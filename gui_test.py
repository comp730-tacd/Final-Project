...
class Main_Menu():
    def __init__(self):
        ...
        self.menu_frame = tk.Frame(self.root, padx=50, pady = 50, bg = "#4E4187", relief= tk.SUNKEN)  

        self.menu_canvas = tk.Canvas(self.menu_frame, width=1000, height=778, bg="black")

        self.menu_canvas.grid(row=1, column=1)

        self.pbg = tk.PhotoImage(file = "assets/house.png")

        self.pic_canvas = tk.Canvas(self.menu_frame, width=1000, height=470)
        self.micro_canvas = tk.Canvas(self.menu_frame, width=498, height=230, bg="#929493")
        self.macro_canvas = tk.Canvas(self.menu_frame, width=1000, height=70, bg="black")
        self.term_canvas = tk.Canvas(self.menu_frame, width=498, height=230, bg="#5e7ef2")

        self.pic_canvas.create_image(500, 200, image = self.pbg) 

        def new_game():
            self.menu_canvas.grid_forget()        
        ...
        def main_menu():
            self.pic_canvas.grid_forget()
            self.micro_canvas.grid_forget()
            self.macro_canvas.grid_forget()
            self.term_canvas.grid_forget()
            self.menu_canvas.grid(row=1, column=1)       
        ...
        self.new_game_button = tk.Button(self.menu_canvas, text="New Game", padx=38, pady=10, command=new_game, fg="white", bg="#777877") 
        self.menu_new_game = self.menu_canvas.create_window( 70, 15, anchor = "nw", window = self.new_game_button) 
